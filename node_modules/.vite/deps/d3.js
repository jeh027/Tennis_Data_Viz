...
function drag_default() {
  ...
  drag.filter = function(_) {
    return arguments.length ? (filter3 = typeof _ === "function" ? _ : constant_default(!!_), drag) : filter3;
  };
...
function brush(dim) {
  var extent3 = defaultExtent, filter3 = defaultFilter2, touchable = defaultTouchable2, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
  function brush2(group3) {
    ...
    overlay.enter().append("rect").attr("class", "overlay").attr("pointer-events", "all").attr("cursor", cursors.overlay).merge(overlay).each(function() {
      ...
      select_default(this).attr("x", extent4[0][0]).attr("y", extent4[0][1]).attr("width", extent4[1][0] - extent4[0][0]).attr("height", extent4[1][1] - extent4[0][1]);
    });
  ...
  brush2.move = function(group3, selection, event) {
    if (group3.tween) {
  ...
  function redraw() {
    var group3 = select_default(this), selection = local2(this).selection;
    if (selection) {
      group3.selectAll(".selection").style("display", null).attr("x", selection[0][0]).attr("y", selection[0][1]).attr("width", selection[1][0] - selection[0][0]).attr("height", selection[1][1] - selection[0][1]);
  ...
  function started(event) {
    ...
    if (!filter3.apply(this, arguments))
      return;
    var that = this, type2 = event.target.__data__.type, mode3 = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local2(that), extent4 = state.extent, selection = state.selection, W = extent4[0][0], w0, w1, N = extent4[0][1], n0, n1, E = extent4[1][0], e0, e1, S = extent4[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
    ...
    e1 = e0;
    s1 = s0;
    var group3 = select_default(that).attr("pointer-events", "none");
  ...
  brush2.extent = function(_) {
    return arguments.length ? (extent3 = typeof _ === "function" ? _ : constant_default2(number22(_)), brush2) : extent3;
  };
  brush2.filter = function(_) {
    return arguments.length ? (filter3 = typeof _ === "function" ? _ : constant_default2(!!_), brush2) : filter3;
  };
...
Path.prototype = path.prototype = {
  ...
  arcTo: function(x12, y12, x22, y22, r) {
    ...
    if (this._x1 === null) {
      ...
      var x20 = x22 - x06, y20 = y22 - y06, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi2 - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
  ...
  arc: function(x4, y4, r, a0, a1, ccw) {
    ...
    if (da2 > tauEpsilon) {
      ...
      this._ += "A" + r + "," + r + ",0," + +(da2 >= pi2) + "," + cw + "," + (this._x1 = x4 + r * Math.cos(a1)) + "," + (this._y1 = y4 + r * Math.sin(a1));
    }
  },
...
function bisector2(f) {
  ...
  if (f.length !== 2) {
    ...
    compare1 = f === ascending2 || f === descending2 ? f : zero2;
    compare2 = f;
    delta = f;
  }
  function left2(a4, x4, lo = 0, hi = a4.length) {
...
function contours_default() {
  ...
  contours.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default4(slice4.call(_)) : constant_default4(_), contours) : threshold2;
  };
...
function density_default() {
  ...
  density.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default4(slice4.call(_)) : constant_default4(_), density) : threshold2;
  };
...
function link_default(links) {
  var id = index3, strength = defaultStrength, strengths, distance = constant_default5(30), distances, nodes, count4, bias, random, iterations2 = 1;
  if (links == null)
    links = [];
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a4 = a0 + a1, b = b0 + b1, c6 = c0 + c1, m3 = sqrt2(a4 * a4 + b * b + c6 * c6), phi2 = asin(c6 /= m3), lambda22 = abs3(abs3(c6) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a4), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function sum_default(value) {
  return this.eachAfter(function(node) {
    var sum5 = +value(node.data) || 0, children = node.children, i = children && children.length;
    while (--i >= 0)
      sum5 += children[i].value;
    node.value = sum5;
  });
...
function stratify_default() {
  var id = defaultId, parentId = defaultParentId, path3;
...
function arc_default() {
  var innerRadius = arcInnerRadius, outerRadius = arcOuterRadius, cornerRadius = constant_default8(0), padRadius = null, startAngle = arcStartAngle, endAngle = arcEndAngle, padAngle = arcPadAngle, context = null, path3 = withPath(arc);
  function arc() {
    ...
    if (!(r1 > epsilon8))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon8 && (padRadius ? +padRadius.apply(this, arguments) : sqrt3(r0 * r0 + r1 * r1)), rc = min4(abs4(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t0, t1;
      if (rp > epsilon8) {
...
function line_default2(x4, y4) {
  var defined = constant_default8(true), context = null, curve = linear_default, output = null, path3 = withPath(line);
...
function area_default5(x06, y06, y12) {
  var x12 = null, defined = constant_default8(true), context = null, curve = linear_default, output = null, path3 = withPath(area);
...
function pie_default() {
  var value = identity_default5, sortValues = descending_default, sort3 = null, startAngle = constant_default8(0), endAngle = constant_default8(tau6), padAngle = constant_default8(0);
  function pie(data) {
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection, k2, p, event) {
    zoom.transform(selection, function() {
      var e = extent3.apply(this, arguments), t0 = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t0.invert(p02), k1 = typeof k2 === "function" ? k2.apply(this, arguments) : k2;
